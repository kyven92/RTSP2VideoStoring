...
int main(void)
{
    ...
    */
    ...
    //open rtsp
    if (avformat_open_input(&input_ctx, "rtsp://admin:planysch4pn@192.168.0.115:554", NULL, &dicts) != 0)
    {
        return EXIT_FAILURE;
    }
    ...
    std::cout << "Initial no. of streams: " << input_ctx->nb_streams << std::endl;
    ...
    while (startMain)
    {
        ...
        if (nRecvPacket >= 0)
        {
            if (packet.stream_index == vidx) // video frame
            {
                ...
                {
                    packet.duration = av_rescale_q(packet.duration, input_ctx->streams[packet.stream_index]->time_base, vstream->time_base);
                    // packet.duration *= 3;
                    ts += packet.duration;

                    packet.pos = -1;
                    // std::cout << "Time Duration: " << ts << std::endl;
                }
                // nGap += 333;
                cnt++;
